...
Or install directly with Go (replace `<version>` with the latest tag from the [releases page](https://github.com/i-am-fran/markdown-do/releases){:target="_blank" rel="noopener noreferrer"}, e.g. `@3.2.0` — plain `@latest` doesn't resolve correctly since releases are tagged without a leading `v`):
